...
- `PerfionPricesDW` er et kurateret prisfeed og ikke et raadt udtraek:
  ...
  - udstiller faste pivoterede felter for de 20 konfigurerede debitorgruppe/valuta/UoM-kombinationer
  ...
  - felterne for `WEB-DE`, `WEB-DK`, `WEB_NL` og `WEB_NO` slutter med henholdsvis `_DE`, `_DK`, `_NL` og `_NO`
  - `WEB_NO` laeses fra company `SCANPAN Norge`; oevrige priskombinationer laeses fra aktuelt company
